...
const AsignarPersonalCRUD = () => {
  ...
  const fetchTripulaciones = async () => {
    try {
      setLoading(true);
      const response = await api.get('/TripulacionVuelo');
  ...
  const handleAdd = async () => {
    ...
    try {
      setSubmitting(true);
      await api.post('/TripulacionVuelo', {
  ...
  const handleUpdate = async () => {
    ...
    try {
      setSubmitting(true);
      await api.put(`/TripulacionVuelo/${vueloID}/${personalID}`, {
        vueloID: parseInt(vueloID),
        personalID: parseInt(personalID),
        rolAsignado: puesto.trim(),
      });
  ...
  const handleDelete = async (vueloId, personalId) => {
    ...
    try {
      await api.delete(`/TripulacionVuelo/${vueloId}/${personalId}`);
      fetchTripulaciones();
    } catch (error) {
